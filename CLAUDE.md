...
- `< 0.30` — discard silently, no comment
- `0.30–0.40` — post low-confidence nudge comment, do not write record
- `0.40–0.65` — write record with `⚠️ Low confidence` flag in markdown
- `> 0.65` — write record normally
...
5. If `contains_decision` is False:
   - If confidence in `[0.30, 0.40)` → post nudge comment via `gh pr comment`
